...
class UserFavoriteCastMemberViewModelTest: KoinTest {
    ...
    private val personList = listOf(
        Person(1, "Emmanuel", Country("mex"), "10-10-10", InnerImages("url","url")),
        Person(2, "Peter", Country("us"), "10-01-10", InnerImages("url","url"))
    )
    ...
    fun setup() {
        ...
        Mockito.`when`(repository.getPersons()).thenReturn(flowOf(personList))
        viewModel = UserFavoriteCastMemberViewModel(repository)
    }
    ...
    @Test
    fun getPersons() {
        viewModel.getPersons().observeForever {
            it.forEach(::println)
